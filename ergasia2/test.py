...
def lcs(search , target):
	...
	#construct empty matrix of size m+1 by n+1
	...
	for i in range(m+1):
		for j in range(n+1):
            ...
            #For zeroes we put zero, for distance of < 200m we +1 and if they are not the same
            #its is the maximum of the LCS of the two adjacent cells
			if i == 0 or j == 0 :
				L[i][j] = 0
			elif x <= 0.2:
				L[i][j] = L[i-1][j-1]+1
